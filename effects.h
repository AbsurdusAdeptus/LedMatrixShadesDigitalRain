...
struct DigitalRainDrop {
  float yIncr;
  float xPos;
  float yPos;
};

DigitalRainDrop digiDrops[10];
...
void digitalRain() {
  ...
  fadeAllPWM();
  for (int i = 0; i < 10; i++) {
    if (abs(digiDrops[i].yIncr) < 0.02) {
      digiDrops[i].xPos = random(0, 24);
      digiDrops[i].yPos = 0;
      digiDrops[i].yIncr = random(10, 60) / 100.0;
    }

    digiDrops[i].yPos += digiDrops[i].yIncr;

    int xPos = (int)digiDrops[i].xPos;
    int yPos = (int)digiDrops[i].yPos;
    if (yPos > 7) {
      digiDrops[i].yIncr = 0;
    } else {
